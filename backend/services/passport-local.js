...
passport.use("local-sign-up", new LocalStrategy(
	...
	async (req, email, password, done) => {
		pool.query("SELECT count(id) as 'user_exists' from user WHERE email = ?;", [email], (err, results) => {
			...
			if(results[0].user_exists > 0) {
				done(null, false, { message: "An account associated with that email already exists.", code: "ERR_SIGN_UP_DUPLICATE", status: 409 });
				return;
			}
			else {
				bcrypt.hash(password, saltRounds, (err, hash) => {
					if(err) {
						done(null, false, { code: "ERR_SIGN_UP_HASH" });
						return;
					}

					pool.getConnection((err, connection) => {
						if (err) {
							done(null, false, { code: "ERR_SIGN_UP_CONNECTION" });
							return;
						}
						...
						connection.beginTransaction((err) => {
							...
							connection.query("INSERT INTO user (email, password, name) VALUES (?, ?, ?);", [email, hash, req.body.name], (err) => {
								if (err) {
								...
								connection.query("SELECT name, email, id FROM user WHERE email = ?", [email], (err, results) => {
									if (err) {
										return connection.rollback(function () {
											connection.release();
